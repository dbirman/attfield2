...
class readout_data:
    ...
    def __init__(self, acts_dist, acts_focl, layer):
        ...
        for i_cat in range(n_cats):
            ...
            self.pos_dist.append(dist_dot[1::2, :, :, :])
            self.neg_dist.append(dist_dot[::2,  :, :, :])
            focl_dot =   cued[layer][i_cat] #* regs[c].w.detach().numpy()[..., None, None]
            self.pos_focl.append(focl_dot[1::2, :, :, :])
            self.neg_focl.append(focl_dot[::2,  :, :, :])
            fake_cued = uncued[layer][i_cat] * eff_gain[i_cat]
            fake_dot = fake_cued #* regs[c].w.detach().numpy()[..., None, None]
            self.pos_fake.append(fake_dot[1::2, :, :, :])
            self.neg_fake.append(fake_dot[::2,  :, :, :])
            fake_undo = cued[layer][i_cat] / eff_gain[i_cat]
            undo_dot = fake_undo #* regs[c].w.detach().numpy()[..., None, None]
            self.pos_undo.append(undo_dot[1::2, :, :, :])
            self.neg_undo.append(undo_dot[::2,  :, :, :])
...
def reconstructed_bhv(
    ...
    ax.scatter(0 + jtr_dist, roc_dist, color = '.4',
        **pkws.bhv_cat)
    ax.scatter(1 + jtr_fake, roc_fake, color = 'C4',
        **pkws.bhv_cat)
    ax.scatter(2 + jtr_focl, roc_focl, color = 'C0',
        **pkws.bhv_cat)
    ax.scatter(3 + jtr_undo, roc_undo, color = 'C2',
        **pkws.bhv_cat)
    ...
    # mean and ci
    ax.plot([0, 0], ci_dist, color = '.2', zorder = 2, **pkws.bhv_ci)
    ax.plot([1, 1], ci_fake, color = '.2', zorder = 2, **pkws.bhv_ci)
    ax.plot([2, 2], ci_focl, color = '.2', zorder = 2, **pkws.bhv_ci)
    ax.plot([3, 3], ci_undo, color = '.2', zorder = 2, **pkws.bhv_ci)
