...
class params:
    ...
    # parameters: IO
    output = 'plots/figures/fig2/fig-reconstruct_2-poster.pdf'
...
def reconstructed_bhv(
    ...
    ax.scatter(0 + jtr_dist, roc_dist, color = '.6',
        **pkws.bhv_cat)
    ax.scatter(1 + jtr_focl, roc_focl, color = '.6',
        **pkws.bhv_cat)
    ax.scatter(2 + jtr_undo, roc_undo, color = 0.45 * mc.to_rgba_array(pkws.pal_bhv[1]) + 0.5 * np.ones(4),
        **pkws.bhv_cat)

    # connecting lines
    ax.add_collection(LineCollection(
        np.stack([
            np.stack([1 + jtr_focl, 2 + jtr_undo]).T,
    ...
    ax.plot([1, 1], ci_focl, color = '.3', zorder = 2, **pkws.bhv_ci)
    ax.plot([2, 2], ci_undo, color = '.3', zorder = 2, **pkws.bhv_ci)
    ax.scatter([0], [roc_dist.mean()], color = '.2', zorder = 3, **pkws.bhv_mean)
    ax.scatter([1], [roc_focl.mean()], color = '.2', zorder = 3, **pkws.bhv_mean)
    ax.scatter([2], [roc_undo.mean()], color = pkws.pal_bhv[1], zorder = 3, **pkws.bhv_mean)
...
ax_c.set_xticklabels([
    pkws.labels.reconst_models[0],
    pkws.labels.reconst_models[2],
    pkws.labels.reconst_models[3]], rotation = 45, ha = 'center')
