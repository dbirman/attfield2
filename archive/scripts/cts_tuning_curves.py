...
if __name__ == '__main__':
    ...
    inputs = {
        'color': video_gen.color_rotation(FRAME+2*MAX_SPACE, 128),
        'fast-sine': video_gen.sine_rotation(FRAME++2*MAX_SPACE, 64,
            freqs = [12]),
        'slow-sine': video_gen.sine_rotation(FRAME++2*MAX_SPACE, 64,
            freqs = [3])
    }
    groups = {
        'color': np.linspace(0, 2*np.pi, 128),
        'fast-sine': np.linspace(0, np.pi, 64),
        'slow-sine': np.linspace(0, np.pi, 64),
    }
    ...
    for k, result in acts.items():
        ...
        if "sine" in k:
            ...
            custom_x = plot.tuning.colorbar_x
        plot.tuning.curve1d(
