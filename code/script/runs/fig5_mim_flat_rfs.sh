...
#SBATCH --gres=gpu:1
#SBATCH --output=/scratch/users/kaifox/attfield/data/runs/fig5/mF-rf_%a.out
#SBATCH --error=/scratch/users/kaifox/attfield/data/runs/fig5/mF-rf_%a.err
ml python/3.6.1
...
echo ">> Starting:" $(date +"%T") " : J=$J : BETA=$BETA" \
     > $DATA/runs/fig5/mF-rf_$J.py_out
F="$DATA/models/fields/field_flat_b$BETA.h5"
$py3 $CODE/script/backprop.py \
    $DATA/runs/fig5/pilot_rfs_mim_flat_beta_$BETA.h5    `# Output Path` \
    ...
    --cuda                                              `# Run on GPU` \
    >> $DATA/runs/fig5/mF-rf_$J.py_out \
    2> $DATA/runs/fig5/mF-rf_$J.py_err
