...
#SBATCH --gres=gpu:1
#SBATCH --output=/scratch/users/kaifox/attfield/data/runs/fig5/sns3-rf_%a.out
#SBATCH --error=/scratch/users/kaifox/attfield/data/runs/fig5/sns3-rf_%a.err
ml python/3.6.1
...
echo ">> Starting:" $(date +"%T") " : J=$J : BETA=$BETA" \
     > $DATA/runs/fig5/sns3-rf_$J.py_out
$py3 $CODE/script/backprop.py \
    $DATA/runs/fig5/pilot_rfs_sens_l3_beta_$BETA.h5     `# Output Path` \
    ...
    --batch_size 1 `# = 1 imgs / gig mem`               `# Limit memory` \
    --verbose                                           `# Debug` \
    --cuda                                              `# Run on GPU` \
    >> $DATA/runs/fig5/sns3-rf_$J.py_out \
    2> $DATA/runs/fig5/sns3-rf_$J.py_err
