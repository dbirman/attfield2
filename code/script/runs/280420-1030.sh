...
#SBATCH --gres=gpu:1
#SBATCH --output=/scratch/users/kaifox/attfield/data/runs/280420/1030_%a.out
#SBATCH --error=/scratch/users/kaifox/attfield/data/runs/280420/1030_%a.err
ml python/3.6.1
cd $HOME/proj/attfield
source code/script/shrlk.bash_rc

#
# Measure effects of retinal flat gain on raidally distributed receptive fields 
#
...
echo ">> Starting:" $(date +"%T") " : J=$J : BETA=$BETA" \
     > $DATA/runs/280420/1030_$J.py_out
$py3 $CODE/script/backprop.py \
    $DATA/runs/280420/rfs_flat_beta_$BETA.h5            `# Output Path` \
    ...
    --cuda                                              `# Run on GPU` \
    >> $DATA/runs/280420/1030_$J.py_out \
    2> $DATA/runs/280420/1030_$J.py_err
