...
class params:
    ...
    # parameters: IO
    output = 'plots/figures/fig5/fig-mim_gauss-size-poster.pdf'
...
lineplots.mini_lineplot(
    ...
    xlim = pkws.lineplot_xlim, ylim = params.size_lim, xticks = [0, 150],
    pkws = pkws)
for ax in ax_c:
    ax.set_ylim(params.size_lim)
    ax.set_yticks(params.size_lim)
    ax.set_yticklabels(["", ""])
...
lineplots.lineplot(
    ...
    pkws = pkws)
ax_c.set_yticks([params.size_lim[0], params.size_lim[1]])
ax_c.set_xticks([0, 150])
sns.despine(ax = ax_c, trim = True, offset = 5)
util.labels(ax_c, pkws.labels.unit_distance, pkws.labels.rf_size, pkws = pkws)
