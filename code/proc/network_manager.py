...
class LayerMod(object):
    ...
    def pre_layer(self, *args, **kwargs):
        ...
        print('running Identity pre')
        return args, kwargs
    def post_layer(self, outputs):
...
class NetworkManager(object):
    ...
    def pre_forward(self, args, kwargs):
        ...
        for mod in self.mods_to_apply(self.full_idx):
            args, kwargs = mod.pre_layer(*args, *kwargs)
        return args, kwargs
    
    def post_forward(self, module, outputs):
        ...
        for mod in self.mods_to_apply(self.full_idx):
            outputs = mod.post_layer(outputs)
...
def patch(klass):
    ...
    if not hasattr(klass.__call__, 'old'):
        ...
        def call_wrapper(self, *args, **kwargs):
            ...
            args, kwargs = mgr.pre_forward(args, kwargs)
            ret = call_wrapper.old(self, *args, **kwargs)
            ret = mgr.post_forward(self, ret)
            return ret
