...
#SBATCH --output=/scratch/users/kaifox/attfield/data/runs/fig5/logs/sn1_rf_%a.out
#SBATCH --error=/scratch/users/kaifox/attfield/data/runs/fig5/logs/sn1_rf_%a.err
...
LOG="$DATA/runs/fig5/logs/sn1_rf_b$BETA"
...
$py3 $CODE/script/backprop.py \
    $DATA/runs/fig5/sn1_rf_n${N_IMG_PER_CAT}_b$BETA.h5     `# Output Path` \
    ...
    --batch_size 300  `# on 16g gpu`                    `# Limit memory` \
    --verbose                                           `# Debug` \
    --cuda                                              `# Run on GPU` \
     > $LOG.py_out \
    2> $LOG.py_err
